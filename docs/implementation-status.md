...
- `npm test`：146 项通过，包含在独立 PGlite/Postgres 环境执行所有迁移和事务回滚测试，以及完整目录采集、范围选择、多门店报价隔离、逐店计划与失败重试、文件并发保护、目录身份、持久化队列（去重入队、退避重试、过期租约回收、许可撤销回滚、响应丢失保护）和调度路由（任务上限、领取截止、空队列、失败上报、鉴权与参数校验）测试。
